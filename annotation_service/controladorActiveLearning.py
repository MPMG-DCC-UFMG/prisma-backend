...
class Classification(object):
    ...
    def start(self):
        ...
        # Rotulação inicial insuficiente para gerar os conjuntos inicial e de teste
        if self.n_initial*3 <= len(labeled_idx):
            ...
            self.flag_rotulosPendentes = self.n_initial*3-len(labeled_idx)
            self.accuracy_scores = [0.0]
...
def createModel():
    ...
    c = Classification(features["id"], PATH_data + features["jsonFileName"], dict_modelsNames[features["model"]], dict_strategies[features["strategy"]], features["n_initial"], features["batch_size"], features["topics"], PATH_results)
    
    dump(c, f'models/{features["id"]}.joblib')
    dict_models[features["id"]] = c
...
def editModel():
    ...
    if features["id"] not in dict_models:
        return Response(
            "Não há modelo vinculado ao identidificar '"+ id + "'.",
            status=400,
        )
    
    c = dict_models[features["id"]]
    ret = c.editModel(features["id"], PATH_data + features["jsonFileName"], dict_modelsNames[features["model"]], dict_strategies[features["strategy"]], features["n_initial"], features["batch_size"], features["topics"])
    dump(c, f'models/{features["id"]}.joblib')
